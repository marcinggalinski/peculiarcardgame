...
namespace PeculiarCardGame.UnitTests.Controllers.DecksController
{
    public class SearchCards
    {
        ...
        {
            var message = await _client.GetAsync($"/api/decks/{_notExistingDeck.Id}/cards/search?query={MatchingQuery}");
        ...
        {
            var message = await _client.GetAsync($"/api/decks/{_existingDeck.Id}/cards/search?query={NotMatchingQuery}");
        ...
        {
            var message = await _client.GetAsync($"/api/decks/{_existingDeck.Id}/cards/search?query={MatchingQuery}");
            var response = await message.Content.ReadFromJsonAsync<List<GetCardResponse>>();
        ...
        {
            await _client.GetAsync($"/api/decks/{_existingDeck.Id}/cards/search?query={query}");
